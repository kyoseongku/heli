...
// (4 triangle face)(3 vertices/triangle) + (1 square face)(2 triangles/face)(3 vertices/triangle)
...
// Vertices of pyramid centered at origin, base = unit square, height = 1
point4 verticesP[5] = {
	point4(0.0, 1.0, 0.0, 1),
	point4(0.5, 0.0, 0.5, 1),
	point4(0.5, 0.0, -0.5, 1),
	point4(-0.5, 0.0, 0.5, 1),
	point4(-0.5, 0.0, -0.5, 1)
};

// Handles each triangular face and assigns normals and texture coordinates to the vertices
int IndexP = 0;
void pyramidTriangle(int a, int b, int c, const point3& normal)
{
	pyramidPoints[IndexP] = verticesP[a];
	pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(0.5, sqrt(1.25));
	IndexP++;

	pyramidPoints[IndexP] = verticesP[b];
	pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(0, 0);
	IndexP++;

	pyramidPoints[IndexP] = verticesP[c];
	pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(0, 1);
	IndexP++;
}

// Handles the bottom square face and assigns normals and texture coordinates to the vertices
void pyramidSquare(int a, int b, int c, int d, const point3& normal)
{
	pyramidPoints[IndexP] = verticesP[a]; pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(0, 1); IndexP++;
	pyramidPoints[IndexP] = verticesP[b]; pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(0, 0); IndexP++;
	pyramidPoints[IndexP] = verticesP[c]; pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(1, 0); IndexP++;
	pyramidPoints[IndexP] = verticesP[a]; pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(0, 1); IndexP++;
	pyramidPoints[IndexP] = verticesP[c]; pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(1, 0); IndexP++;
	pyramidPoints[IndexP] = verticesP[d]; pyramidNormals[IndexP] = normal;
	pyramidUV[IndexP] = point2(1, 1); IndexP++;
}

void pyramid()
{
	pyramidTriangle(0, 1, 2, point3(cross(verticesP[0]-verticesP[2], verticesP[1]-verticesP[2])));
	pyramidTriangle(0, 3, 1, point3(cross(verticesP[0]-verticesP[1], verticesP[3]-verticesP[1])));
	pyramidTriangle(0, 4, 3, point3(cross(verticesP[0]-verticesP[3], verticesP[4]-verticesP[3])));
	pyramidTriangle(0, 2, 4, point3(cross(verticesP[0]-verticesP[4], verticesP[2]-verticesP[4])));
	pyramidSquare(1, 3, 4, 2, point3(0, -1, 0));
}
